        ...
        if((request.session.get('userEmail'))!=None or ((request.user!=None) and (len(str(request.user.username).strip())) != 0)):
            # try: 
                ...
                for u_gig in active_gigs_details:
                    ...
                    if(gig_image != None):
                        gig_image_url = gig_image.gig_image
                    active_gigs_data.append({"gig_id":u_gig.pk,"gig_Name":u_gig.gig_title,"gig_Image":gig_image_url,"gig_user":u_gig.user_id,"gig_username":u_gig.user_id.username})
                ...
                for search in search_term:
                    try:
                        subcate_inst = SubSubCategories.objects.filter(sub_sub_category_Name__contains = search["search_words"])
                        for subcat in subcate_inst:
                            sear_gig_data = UserGigs.objects.get(gig_sub_category=subcat)
                        ...
                        gigs_inst = UserGigs.objects.filter(gig_title__contains = search["search_words"])
                        for sear_gig_data in gigs_inst:
                            search_gig_image_url = ''
                            sea_gig_image = Usergig_image.objects.filter(package_gig_name=sear_gig_data).first() 
                            if(sea_gig_image != None):
                                search_gig_image_url = sea_gig_image.gig_image
    ...
    def get(self , request,username='',orderid=0):
        if((request.session.get('userEmail'))!=None or ((request.user!=None) and (len(str(request.user.username).strip())) != 0)):
            # try:
                ...
                order_details = User_orders.objects.get(order_no = orderid)
                offer_details =  Request_Offers.objects.get(id = order_details.offer_id.id)
                transaction_details = User_Transactions.objects.get(offer_id = offer_details,order_no= order_details)
                ordered_by_user = User.objects.get(id= order_details.order_by.id)
                ordered_to_user = User.objects.get(id= order_details.order_to.id)
                extra_offer = User_orders_Extra_Gigs.objects.filter(order_no=order_details)
                current_user = ''
        ...
        try:
            ...
            if(Buyer_Reviews.objects.filter(order_no=ord_details,b_review_from=orderedto_user,b_review_to=orderedby_user,package_gig_name= gig_details).exists() == False):
                ...
                if(notification_buyer.is_active == True):
                    noti_create = CustomNotifications(sender = orderedto_user, recipient=orderedby_user,order_no= ord_details, verb='reviews',description= str(orderedto_user.username).title() + " left a " +str(round(b_rating)) + " star review.")  
                    noti_create.save()
            data.append({'sucess' : 'sucess'})
        except Exception as e:
            data.append({"error" : str(type(e)) + str(e)})
...
def get_message_notify_view(request):
    if request.method == 'GET':
        ...
        if(len(o_username.strip()) != 0):
            try:
                userDetails = User.objects.get(username = o_username)
                all_nots_list = CustomNotifications.objects.filter(recipient=userDetails).order_by('-pk')
                last_message_time = ''
                unread_count = 0
                username_list = []
                order_username_list = []
    ...
    if request.method == 'GET':
        ...
        if(len(o_username.strip()) != 0):
            try:
                userDetails = User.objects.get(username = o_username)
                unread_count = 0
                order_nots_list = CustomNotifications.objects.filter(recipient=userDetails).exclude(Q(verb="order_chat") | Q(verb= "chat")).order_by('-pk')
...
def post_view_gig_offer_view(request):
    if request.method == 'GET':
        ...
        offer_details = Request_Offers(gig_name=gig_details,user_id=initiator,buyer_request=None,custom_user=receiver, offer_desc="Its an custom Order", offer_budget=o_text_price, offer_time=o_text_del_time,no_revisions=o_text_no_revs, ask_requirements= ask_req, extra_parameters=str(""),offer_type="custom" )
        offer_details.save()
        return HttpResponse(offer_details.pk)