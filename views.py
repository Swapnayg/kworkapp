...
def post_search_key_view(request):
    if request.method == 'POST':
        ...
        if((request.session.get('userEmail'))!=None or ((request.user!=None) and (len(str(request.user.username).strip())) != 0)):
            userDetails = User.objects.get(pk=request.session.get('userId')  if request.session.get('userId') !=None else request.user.id)
            search_term= UserSearchTerms(search_words=keyword,ip_address = str(ipAddress),search_types=keyword_type,user_id = userDetails )
            search_term.save()
        else:   
            search_term= UserSearchTerms(search_words=keyword,ip_address = str(ipAddress),search_types=keyword_type)
            search_term.save()
        return HttpResponse("sucess")
            ...
            for l_details in learning_topics_Details:
                ...
                try:
                    if(l_details.upload_pdf.url != None):
                        url_link =  str(l_details.upload_pdf.url)
...
def handler500(request):
    ...
    response.status_code = 500
    return response
>>>>>>> a146922a96d75b747f9822cf4aab4c8cce62fe6f
